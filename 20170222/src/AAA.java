...
public class AAA {
	...
	// num1은 private 형태이기 때문에 내부에서만 사용가능한 멤버 변수
	private int num1 = 0;
	...
	// 결과적으로 외부에서의 멤버변수(정보)에 대한 접근을 안전하게 처리할수있다.

	// getter / setter 는 무조건 public 형태로 만든다.
	...
	// 변수의 지역성
	// 같은 코드블럭(중괄호{}_ 안에서는 동일한 이름의 변수를 만들수 없고,
	// 지역변수, 매개변수는 변수는 중괄호를 벗어나면 사라진다.
	// 멤버변수는 객체가 사라질 때 사라진다.
	public void foo() {
		int a; // 만들어지는 시점? foo() 메소드가 호출 되었을 때 만들어진다.
				// 사라지는 시점? 래ㅐ() 메소드가 종료 되었을 때 사라진다.
