public class A {
	...
	// 생성자의 구조
	// 1) 생성자는 무조건 접근지시 제어가자 ""public""
	// -> 객체는 항상 클래스 외부에서 만든다.
	// 때문에 private으로 되어 있으면 외부에서 객체를 만들 수 가 없다.

	// 2) 생성자의 이름은 항상 클래스의 이름과 똑같다. 반환형이 없다.

	// 3) 일반적인 메소드 처럼 매개변수를 지정할 수 있다.

	// 이런 형태의 생성자가 디폴트 생성자 (컴파일러가 만들어 내는)
	/*
	 * public A(){
	 * 
	 * }
	...
	private int num1 = 10; // 생성자에서 초기화가 되지 않아도 된다.
	...
	// 생성자에 매개변수가 있는 경우
	// 보통은 멤버변수를 초기화 하기 위한 매개변수가 설정 된다.
	// 더이상 기본 형태의 생성자(new A()) 의 호출은 불가능 하다.
